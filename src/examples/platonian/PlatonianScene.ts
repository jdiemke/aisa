...
export class PlatonianScene extends AbstractScene {
    ...
    public drawBlenderScene8(framebuffer: Framebuffer, elapsedTime: number): void {
        ...
        const camera: Matrix4f =
            Matrix4f.constructTranslationMatrix(Math.sin(elapsedTime * 0.0002)*39,
            Math.cos(elapsedTime * 0.00032)*22,
                 -134 + (Math.sin(elapsedTime * 0.00007) * 0.5 + 0.5) * 17)
                .multiplyMatrix(
                    Matrix4f.constructXRotationMatrix(elapsedTime * 0.00030).multiplyMatrix(
                    Matrix4f.constructXRotationMatrix(elapsedTime * 0.00030).multiplyMatrix(

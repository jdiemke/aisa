...
class Application {
    ...
    public static main(): void {
        const scene = new OscilloscopeScene();
        const canvas: Canvas = new Canvas(128, 128, scene);
        canvas.init();
        Application.createOscilloscopeControls(canvas, scene);
    }

    private static createOscilloscopeControls(canvas: Canvas, scene: OscilloscopeScene): void {
        ...
        btn.addEventListener('click', async () => {
            ...
            try {
                const blob = await OscilloscopeGenerator.generateWav(
                    canvas.framebuffer,
                    sampleRate,
                    duration,
                    traceHz,
                    maxPoints,
                    Color.WHITE.toPackedFormat() >>> 0,
                ...
                WavEncoder.download(
                    blob,
                    `oscilloscope-${modeTag}-${sampleRate}Hz-${duration}s.wav`
                );

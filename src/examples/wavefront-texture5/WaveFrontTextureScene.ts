...
export class WaveFrontTextureScene extends AbstractScene {
    ...
    public render(framebuffer: Framebuffer, time: number): void {
        const elapsedTime: number = time*0.1;
    ...
    private getModelViewMatrix(elapsedTime: number): Matrix4f {
        const camera: Matrix4f = Matrix4f.constructTranslationMatrix(Math.sin(elapsedTime*0.001)*6, Math.cos(elapsedTime*0.0018)*4.5, -20)
        .multiplyMatrix(
